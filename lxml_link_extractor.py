...
def find_all_watch_links(url: str, session) -> list:
    ...
    try:
        ...
        soup = BeautifulSoup(response.content, 'lxml')  # Using lxml
...
def find_media_links_on_page(url: str, session) -> list:
    """
    Scrapes a 'watch' page to find all actual media links (.mkv, .mp4).
    """
...
if __name__ == "__main__":
    ...
    if all_media_info:
        ...
        with open('scraped-links-lxml.json', 'w') as json_file: # New output file
            json.dump(output_data, json_file, indent=4)
        print(f"\nSuccessfully saved {len(all_media_info)} unique media items to scraped-links-lxml.json")
    else:
